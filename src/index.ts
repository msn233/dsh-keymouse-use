...
import {
  KEYMOUSE_SKILL_NAME,
  KEYMOUSE_SKILL_DESCRIPTION,
  KEYMOUSE_SKILL_WHEN_TO_USE,
  KEYMOUSE_SKILL_CONTENT,
...
class HelperClient {
  ...
  private ensureStarted(): Promise<void> {
    ...
    if (this.started && this.child !== null) return Promise.resolve();
    this.started = true;
  ...
  private consume(line: string) {
    ...
    try {
      const parsed = JSON.parse(line.trim()) as {
        ok?: unknown; error?: unknown; path?: unknown; width?: unknown; height?: unknown;
        hwnd?: unknown; pid?: unknown; title?: unknown; process?: unknown;
        x?: unknown; y?: unknown; w?: unknown; h?: unknown; visible?: unknown;
      };
      ...
      const base: HelperResult = error === undefined ? { ok } : { ok, error };
      if (path !== undefined) base.path = path;
      if (width !== undefined) base.width = width;
      if (height !== undefined) base.height = height;
      if (hwnd !== undefined) base.hwnd = hwnd;
      if (pid !== undefined) base.pid = pid;
      if (title !== undefined) base.title = title;
      if (process !== undefined) base.process = process;
      if (visible !== undefined) base.visible = visible;
  ...
  private failInFlight(err: unknown) {
    const cur = this.inFlight;
    if (cur === null) return;
    this.inFlight = null;
    clearTimeout(cur.timer);
  ...
  send(cmd: Record<string, unknown>): Promise<HelperResult> {
    const run = this.chain.then(async () => {
      await this.ensureStarted();
      if (this.child === null) throw new Error('helper is not running');
  ...
  private writeAndRead(cmd: Record<string, unknown>): Promise<HelperResult> {
    return new Promise((resolve, reject) => {
      const child = this.child!;
      const timer = setTimeout(
        () => this.failInFlight(new Error('helper timed out')),
        this.timeoutMs,
      );
...
function renderOutput(kind: string) {
  return (args: unknown, value: HelperResult) => {
    const a = args as { key?: string; button?: string; x?: number; y?: number };
    const detail =
      a?.key ??
      a?.button ??
      ...
      undefined;
    if (value.ok) {
      return [{ type: 'text' as const, text: detail !== undefined ? `${kind}: ${detail} 完成` : `${kind} 完成` }];
    }
    return [{ type: 'text' as const, text: `${kind}失败: ${value.error ?? '未知错误'}` }];
  };
}

/** 截图结果的 render:成功时显示保存路径与尺寸,失败显示错误。 */
function renderScreenshot(args: unknown, value: HelperResult) {
  if (value.ok) {
    const text = value.path ? `截图已保存: ${value.path} (${value.width}x${value.height})` : '截图完成';
    return [{ type: 'text' as const, text }];
  }
  return [{ type: 'text' as const, text: `截图失败: ${value.error ?? '未知错误'}` }];
}

/** get_window 的 render:显示标题/进程/pid/位置。 */
function renderGetWindow(args: unknown, value: HelperResult) {
  if (value.ok) {
    const tex = `当前窗口: ${value.title ?? '(无标题)'} (${value.process ?? ''}, pid=${value.pid ?? '?'}, ${value.x ?? 0},${value.y ?? 0} ${value.w ?? 0}x${value.h ?? 0})`;
    return [{ type: 'text' as const, text: tex }];
  }
  return [{ type: 'text' as const, text: `get_window失败: ${value.error ?? '未知错误'}` }];
}

/** win_rect 的 render:显示位置与尺寸。 */
function renderWinRect(args: unknown, value: HelperResult) {
  if (value.ok) {
    const tex = `窗口位置: (${value.x ?? 0},${value.y ?? 0}) ${value.w ?? 0}x${value.h ?? 0}`;
    return [{ type: 'text' as const, text: tex }];
  }
  return [{ type: 'text' as const, text: `win_rect失败: ${value.error ?? '未知错误'}` }];
}

/** win_find 的 render:显示匹配窗口的句柄/标题/进程/可见性。 */
function renderFindWindow(args: unknown, value: HelperResult) {
  if (value.ok) {
  ...
  return [{ type: 'text' as const, text: `win_find失败: ${value.error ?? '未知错误'}` }];
}

function apply(ctx: Context, config: KeymouseConfig = {}) {
  ...
  const register = (tool: Parameters<typeof ctx.tools.register>[0]) => ctx.tools.register(tool);
  ...
  register(defineTool({
    ...
    async execute(args: { key: string }, exec) {
  ...
  register(defineTool({
    ...
    async execute(args: { button: string }, exec) {
      if (exec.signal.aborted) throw new Error('aborted');
  ...
  register(defineTool({
    ...
    async execute(args: { x: number; y: number }, exec) {
      if (exec.signal.aborted) throw new Error('aborted');
  ...
  register(defineTool({
    ...
    async execute(args: { up: boolean; amount?: number }, exec) {
      if (exec.signal.aborted) throw new Error('aborted');
  ...
  register(defineTool({
    ...
    async execute(args: { hwnd?: number }, exec) {
      if (exec.signal.aborted) throw new Error('aborted');
  ...
  register(defineTool({
    ...
    async execute(args: { hwnd: number; on: boolean }, exec) {
      if (exec.signal.aborted) throw new Error('aborted');
  ...
  const winState = (action: 'minimize' | 'restore' | 'maximize', hint: string) =>
    defineTool({
      name: `win_${action}`,
      description: hint,
      parameters: {
  ...
  register(defineTool({
    ...
    async execute(args: { process: string }, exec) {
      if (exec.signal.aborted) throw new Error('aborted');
